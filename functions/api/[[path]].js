...
export default {
  async fetch(request, env, ctx) {
    ...
    // 3. API: جلب قنوات تصنيف معين
    if ((pathname.startsWith("/api/categories/") || pathname.startsWith("/categories/")) && pathname.endsWith("/channels")) {
      const parts = pathname.split("/");
      const categoryId = parts[parts.indexOf("categories") + 1] || "1";
      try {
    ...
    // 4. API: جلب تفاصيل قناة منفردة
    if (pathname.startsWith("/api/channel/") || pathname.startsWith("/channel/")) {
      const parts = pathname.split("/");
    ...
    // 5. API: معالجة البث وسحب القطع
    if (pathname.startsWith("/api/stream/") || pathname.startsWith("/stream/")) {
    ...
    return new Response("Yacine Standard Worker Active!", { headers: { "Content-Type": "text/plain" } });
  }
};
